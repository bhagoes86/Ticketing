...
function validateUpload(){
    ...
    var txt = "";
    if ('files' in inputUpload) {
        if (x.files.length == 0) {
            txt = "Pilih satu file gambar";
			document.getElementById("inputUpload-label").style.color = "red"
	        $('#inputUpload-label').text(txt);	  
        } else {
            for (var i = 0; i < x.files.length; i++) {
                txt = "name: " + file.name + "<br>";
            }
			document.getElementById("inputUpload-label").style.color = "white"	        
	        $('#inputUpload-label').text('');
        }     
	}
    else {
        if (x.value == "") {
            ...
            txt = "files property tidak disupport oleh browser Anda";
        }
		document.getElementById("inputUpload-label").style.color = "red"
	    $('#inputUpload-label').text(txt);	      
	  };
